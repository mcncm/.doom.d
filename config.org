...
(defun my/theme-color (color)
  "A generalization of ~my/ewal-color~ that should with for all Doom themes. COLOR should be a key in ~doom-themes--colors~."
  (let ((color-spec                 ;; a list with four elements
         (seq-find
         (lambda (x) (eq (car x) color)) ;; take the matching one
         doom-themes--colors)))
    (car (cdr color-spec))          ;; return the second element
    ))
#+END_SRC
...
A fun little trick and terrible hack to recolor the banner. Maybe I should try ~async-shell-command~ rather than ~shell-command~?
#+BEGIN_SRC elisp
(defun my/recolor-banner ()
  (shell-command
   (concat "sed -e \"s/#000000/"          ;; replace all black strokes…
           (my/theme-color 'blue) "/g\" "  ;; g flag to replace all on line
...
(after! org
  (setq-default
   org-log-done 'time  ;; when a task is finished, log the time
