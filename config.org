...
#+begin_src  emacs-lisp
(setq
  my/nixos-dir "~/.config/home-manager"
...
#+begin_src emacs-lisp
(defun typst-find-project-root ()
  "Find the nearest directory containing typst.toml, starting from the current buffer's directory.
Return nil if no such directory is found."
  (let ((dir (buffer-file-name)))
    (when dir
      ...
      (catch 'found
        (while dir
          (when (file-exists-p (expand-file-name "typst.toml" dir))
            (throw 'found dir))
          ;; Go up one directory level or exit if we're at root
          (let ((parent (file-name-directory (directory-file-name dir))))
            (if (string= parent dir)
                (setq dir nil)  ; We've reached the root directory
              (setq dir parent))))))))

(defun typst-set-watch-options ()
  "Set `typst-ts-watch-options` to the directory containing typst.toml."
  (when (eq major-mode 'typst-ts-mode)
    (let ((project-root (typst-find-project-root)))
      (when project-root
        (setq-local typst-ts-watch-options (format "--root %s" project-root))
        (message "Set typst-ts-watch-options to %s" project-root)))))

;; Add to typst-ts-mode-hook to run automatically when opening typst files
(add-hook 'typst-ts-mode-hook #'typst-set-watch-options)

;; Optionally, provide a command to manually run this function
(defun typst-update-watch-options ()
  "Manually update the typst-ts-watch-options to the project root."
  (interactive)
  (typst-set-watch-options))
#+end_src

** TeX
Broken auctex package workaround
#+begin_src emacs-lisp
...
#+end_src
*** Largo
=largo= is my own personal TeX build tool. Here's a little setup to make it play nice with tex-mode.

Here are some utility functions...
#+begin_src emacs-lisp :

(setq largo/default-profile "dev")

(setq largo/project-file "largo.toml")

(setq largo/output-file "_start.pdf")

(setq largo/output-buffer "*Largo Output*")

(setq largo/error-buffer "*Largo Error*")

(setq largo/display-buffer "*Largo Display*")

(add-to-list 'display-buffer-alist `(,largo/output-buffer display-buffer-no-window (nil)))
(add-to-list 'display-buffer-alist `(,largo/error-buffer display-buffer-no-window (nil)))
(add-to-list 'display-buffer-alist `(,largo/display-buffer (+popup-buffer)))


(defun largo/get-config-from-root (root-dir)
  "Given a root directory `root-dir', return the path to the largo project file therein."
  (file-name-concat root-dir largo/project-file))

;; (defun largo/is-project (dir)
;;   "Returns `true' iff `dir' is a largo project"
;;   (file-exists-p (largo/config-from-root dir)))

(defun largo/get-enclosing-project-root (&optional path)
  "Return the root of an enclosing largo project, or `nil' if there is no enclosing project."
  (let*
      ((path (or path default-directory))
       (largo-file (locate-dominating-file path largo/project-file)))
    (file-name-directory largo-file)))

(defun largo/get-build-directory-from-root (root)
  (file-name-concat root "target" largo/default-profile "build"))

(defun largo/get-output-file-from-root (root)
  (file-name-concat (largo/get-build-directory-from-root root) largo/output-file))

(defun largo/build ()
  (interactive)
  (let*
      ((root-dir (largo/get-enclosing-project-root))
       (output-file (largo/get-output-file-from-root root-dir)))
    (shell-command "largo build")))

(defun largo/get-output-buffer ()
  (get-buffer-create largo/output-buffer))

(defun largo/get-error-buffer ()
  (get-buffer-create largo/error-buffer))

(defun largo/get-display-buffer ()
  (get-buffer-create largo/display-buffer))

(defun largo/async-shell-command (command)
  (async-shell-command command (largo/get-output-buffer) (largo/get-error-buffer)))

(defun largo/run-command (args &optional callback)
  (let
      ((callback (or callback (lambda () ()))))
    (set-process-sentinel
     (apply #'start-process
            "largo-process"
            (largo/get-output-buffer)
            "largo"
            args)
     `(lambda (p e)
...
(defun largo/clean ()
  (interactive)
  (largo/run-command '("clean")))

(defun largo/run-build-command ()
  (interactive)
  (let*
      ((root-dir (largo/get-enclosing-project-root))
       (output-file (largo/get-output-file-from-root root-dir))
       (callback
        `(lambda () (find-file-noselect ,output-file))))
    (largo/run-command '("build") callback)))
#+end_src

...And we'll define a minor mode.
#+begin_src emacs-lisp
(define-minor-mode largo-mode
  "A minor mode for using tex-mode with largo.")
#+end_src

Finally, we'll run a hook to start the mode in TeX mode, and override the keymap to build with largo.
#+begin_src emacs-lisp
(defun my/largo-if-in-project ()
  (let ((root (largo/get-enclosing-project-root)))
    (if root
        (progn
          (largo-mode)
          (setq TeX-output-dir (largo/get-build-directory-from-root root))
          ())
      ())))

(add-hook! 'TeX-mode-hook #'my/largo-if-in-project)
...
Add typst support, piggybacking on markdown
#+begin_src emacs-lisp
(after! citar
   ; TODO
)
